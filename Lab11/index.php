...
<main style="padding: 20px;">
    ...
    <div class="toolbar">
        ...
        <button id="btnNewRoom" style="padding: 8px 20px; background: #2ecc71; color: white; border: none; border-radius: 4px; cursor: pointer; margin-left: 10px;">
            + Додати номер
        </button>
    </div>
...
<script type="text/javascript">
    ...
    dp.onBeforeEventRender = function(args) {
        ...
        args.e.html = args.e.text + " (" + start.toString("dd.MM") + " - " + end.toString("dd.MM") + ")";

        // Логіка кольорів за статусом
        switch (args.e.status) {
            case "New":
                args.e.barColor = (start < today.addDays(1)) ? 'red' : 'orange';
                args.e.toolTip = 'Новий';
                break;
            case "Confirmed":
                args.e.barColor = (start < today) ? "red" : "green";
                args.e.toolTip = "Підтверджено";
                break;
            case 'Arrived':
                args.e.barColor = "#1691f4"; // Блакитний
                args.e.toolTip = "Гість у номері";
                break;
            case 'CheckedOut':
                args.e.barColor = "gray";
                args.e.toolTip = "Виїхав";
                break;
        }

        // Додаємо індикатор оплати в нижню частину блоку
        var paid = args.e.paid || 0;
        args.e.areas = [
            { bottom: 2, right: 4, html: "<div style='color:#666; font-size: 7pt;'>Paid: " + paid + "%</div>"},
            { left: 0, bottom: 0, right: 0, height: 3, html: "<div style='background-color:#aaaaaa; height:100%; width:" + paid + "%'></div>"}
        ];
    };
    ...
    function createRoom() {
        ...
        modal.onClosed = function(args) {
            if (args.result === "OK") loadResources();
        };
    ...
    $("#btnNewRoom").click(function() {
        var modal = new DayPilot.Modal({
            ...
            onClosed: function(args) {
                if (args.result === "OK") {
                    loadResources(); // Перезавантажуємо список кімнат після додавання
                }
            }
        });
        modal.showUrl("room_new.php");
    ...
    dp.allowEventOverlap = false; // Заборона накладання бронювань
    ...
    dp.onEventMoved = function (args) {
        $.post("backend_move.php", {
            ...
            if (data.result === "OK") {
                ...
                dp.message(data.message);
                loadEvents(); // Повертаємо назад, якщо помилка
            }
        });
    };

    dp.eventDeleteHandling = "Update"; // Вмикає значок "х" при наведенні на бронь

    dp.onEventDeleted = function(args) {
        if (confirm("Видалити це бронювання?")) {
            $.post("backend_delete.php", { id: args.e.id() }, function() {
                dp.message("Видалено успішно");
            });
        } else {
            loadEvents(); // Відміняємо видалення в інтерфейсі
        }
    };
